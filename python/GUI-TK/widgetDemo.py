...
class App(Tk):
  ...
  def showVals(self):
    ...
    print "Entry contains this: %s" % self.txtEntry.get()
    ...
    if selected == ():
      ...
      print "List box: %s) %s" % (selected[0], selValue)
    ...
    print "radio: %s" % self.radVar.get()
    
    #get the scale's value
    print "scale: %d" % self.scrScale.get()
    
    #get the spinbox's value
    print "spin: %s" % self.spnSpin.get()
